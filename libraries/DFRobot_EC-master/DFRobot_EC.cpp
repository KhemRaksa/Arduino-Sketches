...
void DFRobot_EC::calibration(float voltage, float temperature,int temp)
{   
    this->_voltage = voltage;
    this->_temperature = temperature;
    // strupr(cmd);
    ecCalibration(temp);                     //if received Serial CMD from the serial monitor, enter into the calibration mode
}
...
void DFRobot_EC::ecCalibration(int mode)
{
    char *receivedBufferPtr;
    static boolean ecCalibrationFinish  = 0;
    static boolean enterCalibrationFlag = 0;
